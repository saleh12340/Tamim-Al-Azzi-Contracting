    ...
    static void backup(Context c){
        ...
        try{
            if(Build.VERSION.SDK_INT>=29){
                ...
                try(InputStream in=new FileInputStream(src);OutputStream out=c.getContentResolver().openOutputStream(u)){
                ...
                try(InputStream in=new FileInputStream(src);OutputStream out=new FileOutputStream(new File(dir,fn))){
                    byte[] buf=new byte[8192];int n;while((n=in.read(buf))>0)out.write(buf,0,n);
                }
            }
        }catch(Exception ignored){}
